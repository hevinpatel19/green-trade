...
const Dashboard = () => {
  ...
  // Derive seller address info from user profile
  ...
  // Auto-run AI forecast when profile address is complete
  useEffect(() => {
  ...
  // Auto-forecast using seller's profile city + state + country
  const runAutoForecast = async () => {
    if (!hasCompleteAddress) return;

    setLoading(true);
    try {
      // Geocode seller's profile city + state + country
      ...
      if (!geoRes.data.length) {
        toast.error("Could not locate your profile city. Please update your address.");
        setLoading(false);
        return;
      }

      const match = geoRes.data[0];
      const displayLoc = `${match.name}, ${match.country}`;
      await processWeather(match.lat, match.lon, displayLoc);
    } catch (e) {
  ...
  const handleListEnergy = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/market/list", {
        ...
        energyAmount: parseFloat(sellAmount),
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ADDRESS REQUIRED GATE */}
        {!hasCompleteAddress ? (
          <div className="bg-white p-12 rounded-2xl shadow-lg border border-amber-200 text-center">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-100 mb-4">
              <span className="text-3xl">📍</span>
            </div>
            <h2 className="text-2xl font-bold text-gray-800 mb-3">Profile Address Required</h2>
            <p className="text-gray-500 max-w-md mx-auto leading-relaxed">
              Complete your profile address (city & country) to unlock AI-powered solar forecasting.
              Your forecast location updates automatically from your profile.
            </p>
            <a href="/profile" className="inline-block mt-6 bg-gray-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-black transition">
              Go to Profile →
            </a>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
              ...
              {/* Auto-forecast info — no manual city input */}
              <div className="space-y-4 bg-gray-50 p-6 rounded-xl border border-gray-200 mb-6">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-500 font-medium">📍 Forecast Location</span>
                ...
                <button onClick={runAutoForecast} disabled={loading} className="w-full bg-gray-900 text-white py-3 rounded-xl font-bold hover:bg-black transition-all">
                  ...
                  <div className="h-48 flex flex-col items-center justify-center border-2 border-dashed rounded-xl bg-gray-50 px-6 text-center">
                    <p className="text-gray-400 font-medium leading-relaxed">
                      {predictionData?.net <= 0
                        ...
                        : "Waiting for AI Forecast to unlock market listing tools."}
                    </p>
                  </div>
                )}
              </div>
